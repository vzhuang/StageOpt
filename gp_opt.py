...
class GaussianProcessOptimization(object):
    ...
    threshold: float or list of floats(up?)
    ...
    def _get_initial_xy(self):
        ...
        for gp in self.gps[1:]:
            if np.allclose(self._x, gp.X):#判断是否相近
                y.append(gp.Y)
            else:
...
class StageOpt(GaussianProcessOptimization):
    ...
    This class implements the `StageOpt` algorithm. It uses a Gaussian
    ...
    >>> opt = StageOpt(gp, parameter_set, fmin=[0.])
    ...
    def __init__(self, gp, parameter_set, fmin, lipschitz=None, beta=2.0,
                 num_contexts=0, threshold=0, switch_time=None, scaling='auto'):
        """Initialization, see `StageOpt`."""
    ...
    def use_lipschitz(self):
        ...
        By default this is set to False, which means the adapted StageOpt
    ...
    def compute_sets(self, full_sets=False):
        ...
        full_sets: boolean
            ...
            computations that are not relevant for running StageOpt
        ...
        for index in sort_index:
            ...
            # uncertain element gets picked by StageOpt anyways, we can
    ...
    def get_new_query_point(self, ucb=False, exploit=False):
        ...
        if exploit:
            ...
            x = self.inputs[self.S, :][np.argmax(u[self.S]), :]
            return x
    ...
    def optimize(self, context=None, ucb=False):
        ...
        return self.get_new_query_point(ucb=ucb, exploit=self.should_exploit()), np.count_nonzero(self.S)

    def should_exploit(self):
        if self.switch_time is None:
            return False
        return True if self.t >= self.switch_time else False
...
class SafeOpt(GaussianProcessOptimization):
    ...
    def optimize(self, context=None, ucb=False):
        ...
        if ucb:
            ...
            self.compute_sets()
        return self.get_new_query_point(ucb=ucb), self.Q, self.S, self.G, self.M
